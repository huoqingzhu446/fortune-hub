...
- **更新** 新增完整的 GitHub Actions CI 工作流配置
- 触发条件：push 到 main/develop/codex/**，或 PR 打开时触发
- 步骤：
  - 安装 Corepack 和 pnpm（版本 10.15.1）
  - 使用 Node.js 22 运行
...
- **更新** 新增完整的 Docker Compose 配置
- 支持 MySQL、Redis、API 服务、管理后台、移动端 H5 和 Nginx 反向代理
- 环境变量配置，包括数据库连接、Redis 连接、API 端口、CORS 配置等
- 健康检查机制，确保服务可用性
- 支持多种部署场景：开发、测试、生产环境
...
Admin --> |"proxy /api"| Api
Mobile --> |"HTTP"| Api
CI["CI 工作流<br/>.github/workflows/ci.yml"] --> Root
DC["Docker Compose<br/>docker-compose.yml"] --> Api
DC --> Admin
DC --> Mobile
```
...
- API 层
  ...
  - 参考路径：[services/api/src/main.ts:1-74](file://services/api/src/main.ts#L1-L74)
- **更新** CI 性能优化
  - 使用 Corepack 确保 pnpm 版本一致性
  - Node.js 22 提供更好的性能和新特性支持
  - 25 分钟超时设置，确保大型项目构建完成
...
## 结论
本规范基于现有仓库配置提炼出统一的开发与协作标准，涵盖代码风格、工作流、评审、命名与组织、组件与 API 设计、数据库标准、性能与安全、测试与 CI 等方面。**更新** 最新的 CI/CD 配置引入了 GitHub Actions、Corepack、Node.js 22 等现代化工具，显著提升了开发体验和构建稳定性。建议团队在实际落地中逐步完善文档与工具链，持续迭代以适应业务演进。
...
### 持续集成与自动化
- **更新** 流水线增强
  - 使用 GitHub Actions 替代传统 CI 工具
  - Corepack 确保 pnpm 版本一致性
  - Node.js 22 提供最新 JavaScript 特性和性能
  - 25 分钟超时设置，支持大型项目构建
