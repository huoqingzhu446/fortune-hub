...
export function buildHexagramLineReadings(input: LineBuildInput): DivinationLineReading[] {
  return LINE_POSITION_CONTENT.map((lineContent, index) => {
    const line = index + 1;
    ...
    return {
      ...
      text: `${label}处于「${input.name}」的${lineContent.theme}位，${lineContent.focus}。本卦主旨为「${input.theme}」，此爻取${lineNature}之象，提示：${line === 3 ? input.caution : input.judgement}`,
      advice: line === 5 ? input.decision : lineContent.action,
      risk: lineContent.risk,
      topicReadings: buildLineTopicReadings(input, line, label, lineContent.action, lineContent.risk),
    };
  });
}

export function buildTopicReading(input: TopicBuildInput): DivinationTopicReading {
  const topicCopy = TOPIC_COPY[input.topic] || TOPIC_COPY.general;
  const movingAdvice = input.movingLineReading?.topicReadings?.[input.topic]?.action || input.movingLineReading?.advice || input.hexagram.decision || '';
...
function buildLineTopicReadings(
  ...
  return Object.entries(TOPIC_COPY).reduce<Partial<Record<DivinationTopic, DivinationTopicReading>>>(
    (result, [topic, topicCopy]) => {
      const typedTopic = topic as DivinationTopic;
      result[typedTopic] = {
        topic: typedTopic,
        title: `${topicCopy.title} · ${label}`,
        summary: `以${topicCopy.lens}来看，「${input.name}」${label}强调${LINE_POSITION_CONTENT[line - 1].focus}。`,
