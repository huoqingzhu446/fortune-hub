...
export class PosterRendererService {
  ...
  async renderPoster(
    ...
    try {
      ...
      const fallbackTemplateMarkup = build(null);
      const imageBuffer = await this.renderPng(fallbackTemplateMarkup);
...
<svg xmlns="http://www.w3.org/2000/svg" width="${layout.width}" height="${layout.height}" viewBox="0 0 ${layout.width} ${layout.height}">
  <defs>
    <linearGradient id="today-overlay" x1="0%" x2="100%" y1="0%" y2="100%">
      <stop offset="0%" stop-color="${colorA}" stop-opacity="0.84" />
      <stop offset="56%" stop-color="${colorB}" stop-opacity="0.36" />
      <stop offset="100%" stop-color="${colorC}" stop-opacity="0.74" />
    </linearGradient>
  ...
  <path d="M108 352 C 248 266, 418 250, 592 322 S 894 424, 996 354" fill="none" stroke="#ffffff" stroke-opacity="0.18" stroke-width="4" />
  <rect x="56" y="56" width="976" height="1360" rx="52" ry="52" fill="#081320" fill-opacity="0.18" stroke="#ffffff" stroke-opacity="0.34" />
  ...
  ${chipBlocks}
  ${metricBlocks}
  <rect x="120" y="862" width="848" height="204" rx="34" ry="34" fill="#0c1724" fill-opacity="0.2" stroke="#ffffff" stroke-opacity="0.18" />
  ...
  ${highlightBlocks}
  <rect x="120" y="1332" width="848" height="80" rx="26" ry="26" fill="#08111a" fill-opacity="0.2" />
...
<svg xmlns="http://www.w3.org/2000/svg" width="${layout.width}" height="${layout.height}" viewBox="0 0 ${layout.width} ${layout.height}">
  ...
  ${chipBlocks}
  <text x="84" y="218" font-size="26" letter-spacing="6" fill="#f4fbff" font-family="SF Pro Display, PingFang SC, sans-serif">FORTUNE HUB WALLPAPER</text>
  <text x="84" y="342" font-size="70" font-weight="760" fill="#ffffff" font-family="SF Pro Display, PingFang SC, sans-serif">${titleTspans}</text>
  <text x="84" y="514" font-size="32" fill="#f8fbff" fill-opacity="0.96" font-family="SF Pro Text, PingFang SC, sans-serif">${subtitleTspans}</text>
  <rect x="84" y="${cardY}" width="${layout.width - 168}" height="${cardHeight}" rx="38" ry="38" fill="url(#glass)" stroke="#ffffff" stroke-opacity="0.24" />
  <text x="118" y="${cardY + 76}" font-size="32" fill="#142634" font-family="SF Pro Text, PingFang SC, sans-serif">${guidanceTspans}</text>
  <text x="84" y="${layout.height - 92}" font-size="25" fill="#ffffff" fill-opacity="0.88" font-family="SF Pro Display, PingFang SC, sans-serif">Generated by Fortune Hub</text>
</svg>`.trim();
  }
